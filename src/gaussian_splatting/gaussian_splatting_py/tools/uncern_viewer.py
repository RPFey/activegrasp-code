...
from gaussian_splatting_py.uncern_render import rasterization_fisher_wrapper, equal_hist
...
def main(local_rank: int, world_rank, world_size: int, args):
    ...
    # TODO: Generate random viewpoints in the free space pointing to the scene to compute Fisher Information
    if os.path.exists(f"{args.data_dir}/transforms.json"):
        # load the Kinova Collected Dataset
        print(f"loading kinova dataset with {args.data_dir}/transforms.json")
        trainset = datasets.KinovaDataset(args.data_dir)
    else:
        parser = Parser(
    ...
    fishers = torch.zeros((means.shape[0],
                           1 + means.shape[1] + colors.shape[1] * colors.shape[2] + scales.shape[1] + quats.shape[1]) # 1 for opacity
                           , device=device)

    for data in tqdm.tqdm(trainloader, total=len(trainset), desc="Computing Fisher Information on Trainset"):
        camtoworlds = data["camtoworld"].to(device)  # [1, 4, 4]
        Ks = data["K"].to(device)  # [1, 3, 3]
        viewmats = camtoworlds.inverse()
        ...
        render_colors, render_alphas, meta = rasterization_fisher_wrapper(
            ...
            sh_degree=sh_degree,
            # TODO 
            fisher_color_scaler=args.color_fisher,
            fisher_depth_scaler=args.depth_fisher
        )
        grad_means3D, grad_sh, grad_opacities, grad_scales, grad_rotations, grad_theta, grad_rho, JTJ_tau = meta["fishers"][0]
        fishers += torch.cat([grad_means3D, 
                             rearrange(grad_sh, "n s c -> n (s c)"),
                               grad_opacities, grad_scales, grad_rotations], dim=1)
        # break

    # TODO: map fishers to color maps
    uncern = torch.reciprocal(fishers + 1e-6)
    H_mean = reduce(uncern, "n c -> n", "mean")
    H_mean = H_mean.cpu().numpy()

    # hist equalization on H_mean
    # H_normalized = equal_hist(H_mean)

    H_max = min(H_mean.max(), H_mean.mean() + 5 * H_mean.std())
    H_min = max(H_mean.min(), H_mean.mean() - 5 * H_mean.std())
    H_normalized = (H_mean - H_min) / (H_max - H_min)
    H_normalized = H_mean / H_mean.max()
    
    cmap = plt.colormaps["inferno"]
    H_color = cmap(H_normalized)[:, :3]
    H_color = torch.from_numpy(H_color).to(colors)
    # H_color = rearrange(torch.from_numpy(H_color).to(colors), "n c -> n 1 c")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--depth-fisher", type=float, default=0.0, help="depth fisher scaler"
    )
    parser.add_argument(
        "--color-fisher", type=float, default=1.0, help="color fisher scaler"
    )
    parser.add_argument(
        "--data_dir", type=str, help="data_dir for colmap parser"
    )
    parser.add_argument("--backend", type=str, default="gsplat", help="gsplat, inria")
    parser.add_argument("--cmap", type=str, default="inferno", help="")
