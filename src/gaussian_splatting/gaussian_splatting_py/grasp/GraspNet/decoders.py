...
class GraspDecoders(Decoders):
    def __init__(self, c_dim=32, hidden_size=16, truncation=0.08, n_blocks=2, learnable_beta=True, fusion_planes = False):
        super().__init__()
        self.fusion_planes = fusion_planes
        if fusion_planes:
            self.g_linears = nn.ModuleList(
                [nn.Linear(6 * c_dim, hidden_size)] +
                [nn.Linear(hidden_size, hidden_size) for i in range(n_blocks - 1)])
        else:
            self.g_linears = nn.ModuleList(
                [nn.Linear(2 * c_dim, hidden_size)] +
                [nn.Linear(hidden_size, hidden_size) for i in range(n_blocks - 1)])
        self.g_output_linear = nn.Linear(hidden_size, 1)

    def get_raw_graspness_seperate(self, p_nor, grasp_planes):
        g_planes_xy, g_planes_xz, g_planes_yz = grasp_planes
        g_feat = self.sample_plane_feature(p_nor, g_planes_xy, g_planes_xz, g_planes_yz)
        h = g_feat
        for i, l in enumerate(self.g_linears):
            h = self.g_linears[i](h)
            h = F.relu(h, inplace=True)
        graspness = self.g_output_linear(h).squeeze()
        return graspness

    def get_raw_graspness_fusion(self, p_nor, all_planes):
        planes_xy, planes_xz, planes_yz, c_planes_xy, c_planes_xz, c_planes_yz, g_planes_xy, g_planes_xz, g_planes_yz = all_planes
        g_feat = self.sample_plane_feature(p_nor, g_planes_xy, g_planes_xz, g_planes_yz)
        c_feat = self.sample_plane_feature(p_nor, c_planes_xy, c_planes_xz, c_planes_yz)
        sdf_feat = self.sample_plane_feature(p_nor, planes_xy, planes_xz, planes_yz)
        h = torch.cat([g_feat,sdf_feat,c_feat],dim=-1)
        for i, l in enumerate(self.g_linears):
            h = self.g_linears[i](h)
            h = F.relu(h, inplace=True)
        graspness = self.g_output_linear(h).squeeze()
    ...
    def forward(self, p, all_planes):
        p_shape = p.shape
        p_nor = normalize_3d_coordinate(p.clone(), self.bound)
        sdf = self.get_raw_sdf(p_nor, all_planes[:6])
        rgb = self.get_raw_rgb(p_nor, all_planes[:6])
        if self.fusion_planes:
            graspness = self.get_raw_graspness_fusion(p_nor, all_planes)
        else:
            graspness = self.get_raw_graspness_seperate(p_nor, all_planes[6:])
        raw = torch.cat([rgb, sdf.unsqueeze(-1), graspness.unsqueeze(-1)], dim=-1)
        raw = raw.reshape(*p_shape[:-1], -1)
        return raw

class GraspDecoders_w_uncertainty(GraspDecoders):
    def __init__(self, c_dim=32, hidden_size=16, truncation=0.08, n_blocks=2, learnable_beta=True, fusion_planes = False):
        super().__init__()
        ...
        self.u_linears = nn.ModuleList(
                [nn.Linear(2 * c_dim, hidden_size)] +
                [nn.Linear(hidden_size, hidden_size) for i in range(n_blocks - 1)])
        self.u_output_linear = nn.Linear(hidden_size, 1)
    ...
    def get_raw_rgb(self, p_nor, all_planes):
        ...
        for i, l in enumerate(self.c_linears):
            h = self.c_linears[i](h)
            h = F.relu(h, inplace=True)
        for i, l in enumerate(self.u_linears):
            t = self.u_linears[i](t)
            t = F.relu(t, inplace=True)

        rgb = torch.sigmoid(self.c_output_linear(h))
        uncertainty = self.u_output_linear(t).squeeze()
        uncertainty = 0.01 + torch.log(1+torch.exp(uncertainty))
    ...
    def forward(self, p, all_planes):
        ...
        p_nor = normalize_3d_coordinate(p.clone(), self.bound)
        sdf = self.get_raw_sdf(p_nor, all_planes[:6])
        rgb, uncertainty = self.get_raw_rgb(p_nor, all_planes[:6])
        if self.fusion_planes:
            graspness = self.get_raw_graspness_fusion(p_nor, all_planes)
        else:
            graspness = self.get_raw_graspness_seperate(p_nor, all_planes[6:])
        raw = torch.cat([rgb, sdf.unsqueeze(-1), graspness.unsqueeze(-1), uncertainty.unsqueeze(-1)], dim=-1)
        raw = raw.reshape(*p_shape[:-1], -1)
        return raw