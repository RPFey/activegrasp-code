...
#
# This file is a modified version of https://github.com/cvg/nice-slam/blob/master/src/config.py
...
import yaml
import os
from .decoders import Decoders, GraspDecoders, GraspDecoders_w_uncertainty
...
def _get_model(cfg):
    ...
    if cfg['model']['grasp_output']:
        if cfg['model']['fusion_plane']:
            decoder = GraspDecoders_w_uncertainty(c_dim=c_dim, truncation=truncation, learnable_beta=learnable_beta, fusion_planes=True)
        else:
            decoder = GraspDecoders_w_uncertainty(c_dim=c_dim, truncation=truncation, learnable_beta=learnable_beta,fusion_planes=False)
    else:
...
def load_config(path, default_path=None):
    ...
    if inherit_from is not None:
        cfg = load_config(os.path.join(PACKAGE_ROOT, inherit_from), default_path)
