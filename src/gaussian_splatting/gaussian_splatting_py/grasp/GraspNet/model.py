...
from gaussian_splatting_py.grasp.GraspNet.utils import batch_viewpoint_params_to_matrix,generate_grasp_views
from gaussian_splatting_py.grasp.GraspNet.MinkUnet import MinkUNet18,MinkUNet50, MinkUNet34_openshape, MinkUNetAdaptor, MinkResNet34
...
def pred_decode_reg(end_points):
    ...
    Returns:
        grasp_preds: [list of torch.FloatTensor, (num_grasps, 17)]
            grasp predictions
            grasp_score (1), grasp_width (1), grasp_height (1), grasp_depth (1), rotation_matrix (9), grasp_center (3), obj_ids (1)
    """
    batch_size = len(end_points['point_clouds'])
    grasp_preds = []
    for i in range(batch_size):
