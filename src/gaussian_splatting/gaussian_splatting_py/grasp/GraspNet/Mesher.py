...
class Mesher(object):
    ...
    def __init__(self, cfg, args, eslam, points_batch_size=500000, ray_batch_size=100000):
        ...
        self.n_img = eslam.n_img # len(self.frame_reader)
    ...
    def get_mesh(self, mesh_out_file, all_planes, decoders, keyframe_dict, device='cuda:0', color=True, graspness = True):
        """
        ...
        with torch.no_grad():
            ...
            for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                z.append(self.eval_points(pnts.to(device), all_planes, decoders).cpu().numpy()[:, 3])
            ...
            if color:
                ...
                z = []
                for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                    z_graspness = self.eval_points(pnts.to(device).float(), all_planes, decoders).cpu()[..., 4]
                    z.append(z_graspness)
                z = torch.cat(z, dim=0)
                z_np = z.numpy()
                z_np = np.clip(z_np, 0, 1)
                plasma_colormap = cm.get_cmap('plasma')
                rgb_color = plasma_colormap(z_np)[:,:3]
