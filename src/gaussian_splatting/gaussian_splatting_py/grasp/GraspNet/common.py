...
def select_uv(i, j, n, b, depths, colors, device='cuda:0', graspnesses = None):
    """
    ...
    if not (graspnesses is None):
        graspnesses = graspnesses.reshape(b, -1)
        graspnesses = torch.gather(graspnesses, 1, indices)  # (b, n)1
    else:
        graspnesses = None

    return i, j, depths, colors, graspnesses

def get_sample_uv(H0, H1, W0, W1, n, b, depths, colors, device='cuda:0', graspnesses= None, downsample = 1):
    """
    ...
    if not (graspnesses is None):
        graspnesses = graspnesses[:, H0:H1, W0:W1]

    i, j = torch.meshgrid(torch.linspace(W0, W1 - 1, int((W1 - W0)/downsample), device=device), torch.linspace(H0, H1 - 1, int((H1 - H0)/downsample), device=device))

    i = i.t()  # transpose
    j = j.t()
    i, j, depth, color, graspnesses = select_uv(i, j, n, b, depths, colors, device=device, graspnesses = graspnesses)

    return i, j, depth, color, graspnesses

def get_samples(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2ws, depths, colors, device, graspnesses = None, downsample = 1):
    """
    ...
    return:
        ...
        sample_depth: (b, n) sampled depth
        sample_color: (b, n, 3) sampled color
        sample_graspness: (b, n) sampled graspness
    """
    b = c2ws.shape[0]
    i, j, sample_depth, sample_color, sample_graspness = get_sample_uv(
        H0, H1, W0, W1, n, b, depths, colors, device=device, graspnesses = graspnesses, downsample= downsample)

    rays_o, rays_d = get_rays_from_uv(i, j, c2ws, H, W, fx, fy, cx, cy, device)

    if not (sample_graspness is None):
        sample_graspness = sample_graspness.reshape(-1)
    return rays_o.reshape(-1, 3), rays_d.reshape(-1, 3), sample_depth.reshape(-1), sample_color.reshape(-1, 3), sample_graspness
...
def get_rays_downsample(H, W, fx, fy, cx, cy, c2w, device, sample_rate = 4):
    """
    Get rays for a whole image.
    ...
    if isinstance(c2w, np.ndarray):
        c2w = torch.from_numpy(c2w)
    # pytorch's meshgrid has indexing='ij'
    i, j = torch.meshgrid(torch.linspace(0, W-1, int(W/sample_rate)), torch.linspace(0, H-1, int(H/sample_rate)))
    i = i.t()  # transpose
    j = j.t()
    dirs = torch.stack(
        [(i-cx)/fx, -(j-cy)/fy, -torch.ones_like(i)], -1).to(device)
    dirs = dirs.reshape(int(H/sample_rate), int(W/sample_rate), 1, 3)
    # Rotate ray directions from camera frame to the world frame
    # dot product, equals to: [c2w.dot(dir) for dir in dirs]
    rays_d = torch.sum(dirs * c2w[:3, :3], -1)
    rays_o = c2w[:3, -1].expand(rays_d.shape)
