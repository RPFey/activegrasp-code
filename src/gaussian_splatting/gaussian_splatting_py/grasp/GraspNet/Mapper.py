...
from .common import (get_samples, random_select, matrix_to_cam_pose, cam_pose_to_matrix)
...
class Mapper(object):
    ...
    def optimize_mapping(self, iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, keyframe_dict,
        ...
        pbar = tqdm.tqdm(total=iters, desc='Mapping Iteration', position=0, leave=True)
        for joint_iter in range(iters):
            ...
            pbar.set_postfix_str(
                f"loss: {loss.item():.2f}, depth_loss: {depth_loss.item():.2f}, color_loss: {color_loss.item():.2f}, "
                f"sdf_loss: {sdf_loss.item():.2f}")
            pbar.update(1)
        ...
        gt_graspness_list.append(cur_gt_graspness)
        gt_graspness = torch.cat(gt_graspness_list, dim=0) # (N, H, W)
    ...
    def mapping_step(self, idx, gt_color, gt_depth, gt_c2w):
        cfg = self.cfg
        # skip first
        # next(data_iterator)
        ...
        gt_color = gt_color.squeeze(0).to(self.device)
        gt_depth = gt_depth.squeeze(0).to(self.device)
        gt_c2w = gt_c2w.squeeze(0).to(self.device)
        cur_c2w = gt_c2w
        ...
        cur_c2w, cur_gt_graspness = self.optimize_mapping(iters, lr_factor, idx, gt_color, gt_depth,
                                                                    gt_c2w, self.keyframe_dict, self.keyframe_list, cur_c2w,
    ...
    def run(self):
        """ Main Running function for mapping. """
        cfg = self.cfg
        all_planes = (
        self.planes_xy, self.planes_xz, self.planes_yz, self.c_planes_xy, self.c_planes_xz, self.c_planes_yz,
        self.g_planes_xy, self.g_planes_xz, self.g_planes_yz)
