...
from gaussian_splatting_py.grasp.vgn_src.transform import Transform
...
class TSDFVolume(object):
    ...
    def __init__(self, size, resolution, origin=np.zeros((3,))):
        self.size = size
    ...
    def integrate(self, depth_img, intrinsic, extrinsic):
        ...
        intrinsic = o3d.camera.PinholeCameraIntrinsic(
            ...
            fx=intrinsic.fx,
            fy=intrinsic.fy,
            cx=intrinsic.ppx if hasattr(intrinsic, "ppx") else intrinsic.cx,
            cy=intrinsic.ppy if hasattr(intrinsic, "ppy") else intrinsic.cy,
        )
    ...
    def get_grid(self):
        '''TSDF to grid
        Returns:
            tsdf_grid: np.ndarray(1, resolution, resolution, resolution)
        '''
        ...
        # TSDF to voxel grid, then convert to list [voxel1, voxel2, ...]
        cloud = self._volume.extract_voxel_point_cloud()
        points = np.asarray(cloud.points)
        distances = np.asarray(cloud.colors)[:, [0]]
        
        # voxel_grid = self._volume.extract_voxel_grid()
        # voxels = voxel_grid.get_voxels()
        
        for idx, point in enumerate(points):
            i, j, k = np.floor( (point - self.origin) / self.voxel_size).astype(int)
            tsdf_grid[0, i, j, k] = distances[idx]
        return tsdf_grid
